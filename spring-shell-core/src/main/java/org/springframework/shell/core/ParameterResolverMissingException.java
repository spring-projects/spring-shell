...
public class ParameterResolverMissingException extends RuntimeException {
	...
	public ParameterResolverMissingException(MethodParameter parameter) {
		super(String.format("No parameter resolver found for parameter with index %d (named '%s') of %s ",
				parameter.getParameterIndex(), parameter.getParameterName(), parameter.getMethod()));
	}
