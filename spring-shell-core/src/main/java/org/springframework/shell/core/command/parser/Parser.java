...
import org.springframework.shell.core.command.parser.Parser.ParseResult.ArgumentResult;
import org.springframework.shell.core.command.parser.Parser.ParseResult.OptionResult;
	...
	class DefaultNodeVisitor extends AbstractNodeVisitor {
			...
			if (!currentOptions.isEmpty()) {
				for (CommandOption currentOption : currentOptions) {
					...
					if (optionPos + 1 < expectedOptionCount) {
						if (currentOption.arityMin() > -1
								&& currentOptionArgument.size() < currentOption.arityMin()) {
						...
						else if (currentOption.arityMin() > -1
								&& currentOptionArgument.size() > currentOption.arityMin()) {
							String arg = currentOption.longName();
		...
		private List<MessageResult> validateOptionNotMissing(Command registration) {
			...
			return requiredOptions2.stream().map(o -> {
				...
				String ins1 = "";
				if (StringUtils.hasText(o.defaultValue())) {
					ins1 = ", " + o.description();
				}
