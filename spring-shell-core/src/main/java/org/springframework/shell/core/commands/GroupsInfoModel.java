...
/**
 * Model encapsulating info about command structure which is more friendly to show and
 * render via templating.
 *
...
class GroupsInfoModel {
	...
	static GroupsInfoModel of(boolean showGroups, Map<String, Command> registrations) {
		...
		List<GroupCommandInfoModel> gcims = commandsByGroupAndName.entrySet().stream().map(e -> {
			List<CommandInfoModel> cims = e.getValue()
				.entrySet()
				.stream()
		...
		boolean hasUnavailableCommands = commands.stream().map(c -> {
			...
			return true;
		}).filter(a -> !a).findFirst().isPresent();
