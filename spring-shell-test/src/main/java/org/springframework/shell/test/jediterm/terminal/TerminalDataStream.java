...
/**
 * Represents data communication interface for terminal. It allows to {@link #getChar()}
 * by one and {@link #pushChar(char)} back as well as requesting a chunk of plain ASCII
 * characters ({@link #readNonControlCharacters(int)} - for faster processing from buffer
 * in the size {@literal <=maxChars}).
 *
